...
function preload() {
  ...
  cannon_water_sound = loadSound("assets/cannon_water.mp3");
...
function showCannonBalls(ball, index) {
  if (ball) {
    ...
    if (ball.body.position.x >= width || ball.body.position.y >= height - 50) {
      if(!isCannonWater && !cannon_water_sound.isPlaying()){
      cannon_water_sound.play();
      cannon_water_sound.setVolume(0.25)
      }
        ball.remove(index);
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        if(!isfunny && !pirate_laugh.isPlaying()){
          pirate_laugh.play();
          isfunny = true;
        }
        isGameOver = true;
...
function keyReleased() {
  if (keyCode === DOWN_ARROW && !isGameOver) {
    if(!isexplode && !cannon_explosion.isPlaying()){
    cannon_explosion.play();
    cannon_explosion.setVolume(0.15)
    }
